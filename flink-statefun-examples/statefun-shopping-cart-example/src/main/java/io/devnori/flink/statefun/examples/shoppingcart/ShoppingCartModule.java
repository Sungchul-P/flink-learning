...
public class ShoppingCartModule implements StatefulFunctionModule {
    ...
    public void configure(Map<String, String> globalConfiguration, Binder binder) {
        ...
        Identifiers ioModule = new Identifiers(kafkaAddress);

        binder.bindIngress(ioModule.getIngressSpecForRestock());
        binder.bindIngressRouter(Identifiers.RESTOCK, new RestockRouter());

        binder.bindIngress(ioModule.getIngressSpecForCart());
        binder.bindIngressRouter(Identifiers.ADD_TO_CART, new AddToCartRouter());

        binder.bindIngress(ioModule.getIngressSpecForCheckOut());
